 ...
 */
package org.training.hospital.cockpits.setup;
...
import org.training.hospital.cockpits.constants.HospitalCockpitsConstants;
 ...
 */
@SystemSetup(extension = HospitalCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(HospitalCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(HospitalCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						HospitalCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/hospitalcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
